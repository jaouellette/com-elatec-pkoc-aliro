...
public final class LeafVerifiedManager
{
    ...
    /**
     * Production LEAF Root CA public key (uncompressed P-256, 65 bytes).
     * This is the LEAF Verified production Root CA used on real MIFARE DUOX cards.
     * Hex: 04 2D 27 81 BE 41 C2 27 58 A6 13 81 0F 67 EC 78
     *      DF 11 76 C4 76 5B 21 2B 49 21 8C 6C 58 40 8A 5A
     *      DA 3D 99 73 20 9D 82 28 91 3A 88 16 97 3C FE 5C
     *      9E BF D8 C6 69 75 32 CD D5 B5 3E E1 34 D2 F1 1B 3C
     */
    public static final String LEAF_PRODUCTION_ROOT_CA_PUB_HEX =
            "042D2781BE41C22758A613810F67EC78DF1176C4765B212B49218C6C58408A5A"
            + "DA3D9973209D8228913A8816973CFE5C9EBFD8C6697532CDD5B53EE134D2F11B3C";

    /** @return the production LEAF Root CA public key as 65 bytes, or null on error. */
    public static byte[] getProductionRootCAPubKey()
    {
        try { return Hex.decode(LEAF_PRODUCTION_ROOT_CA_PUB_HEX); }
    ...
    /**
     * Extract the Open ID from the certificate's subject.
     *
     * Supports two certificate formats:
     *   1. HCE certs: CN = "LEAF-XXXXXXXXXXXX" (12-digit Open ID after prefix)
     *   2. Real DUOX certs: serialNumber (OID 2.5.4.5) = 12-digit Open ID
     *
     * @param certDER DER-encoded certificate
     * @return Open ID string, or null on error
     */
    public static String extractOpenIDFromCert(byte[] certDER)
    {
        try
        {
            ...
            // Try serialNumber (OID 2.5.4.5) first — used by real DUOX cards
            org.bouncycastle.asn1.x500.RDN[] snRdns = subject.getRDNs(BCStyle.SERIALNUMBER);
            if (snRdns != null && snRdns.length > 0)
            {
                String sn = snRdns[0].getFirst().getValue().toString();
                Log.d(TAG, "extractOpenIDFromCert: serialNumber=" + sn);
                if (sn != null && !sn.isEmpty())
                    return sn;
            }

            // Fallback: CN with "LEAF-" prefix — used by HCE credential app
            org.bouncycastle.asn1.x500.RDN[] cnRdns = subject.getRDNs(BCStyle.CN);
            if (cnRdns != null && cnRdns.length > 0)
            {
                String cn = cnRdns[0].getFirst().getValue().toString();
                Log.d(TAG, "extractOpenIDFromCert: CN=" + cn);
                if (cn != null && cn.startsWith("LEAF-"))
    ...
    /**
     * Verify the DUOX INTERNAL AUTHENTICATE signature.
     *
     * The message signed by the DUOX card is:
     *   {@code F0 F0 || OptsA(80 00) || RndB(16 bytes) || RndA(16 bytes)}
     * where:
     *   - {@code F0 F0} is the protocol prefix constant
     *   - OptsA is {@code 80 00} (the options TLV data field, 2 bytes)
     *   - RndB is the 16-byte random nonce generated by the card
     *   - RndA is the 16-byte challenge generated by the reader
     *
     * The signature is raw R||S (64 bytes), which must be converted to DER
     * before passing to Java's {@link Signature#verify}.
     *
     * @param cardRandom16  16-byte random nonce from the card's TLV response (tag 81)
     * @param challenge16   16-byte random challenge sent by the reader
     * @param sigRS64       64-byte raw R||S signature from the card's TLV response (tag 82)
     * @param credPubKey65  65-byte uncompressed credential public key from the cert
     * @return true if the DUOX signature is valid
     */
    public static boolean verifyDuoxSignature(
            byte[] cardRandom16, byte[] challenge16,
            byte[] sigRS64,      byte[] credPubKey65)
    {
        try
        {
            Security.addProvider(new BouncyCastleProvider());

            // Build the 36-byte message: F0 F0 || 80 00 || RndB(16) || RndA(16)
            byte[] msg = new byte[36];
            msg[0] = (byte)0xF0;
            msg[1] = (byte)0xF0;
            msg[2] = (byte)0x80;
            msg[3] = (byte)0x00;
            System.arraycopy(cardRandom16, 0, msg, 4,  16);
            System.arraycopy(challenge16,  0, msg, 20, 16);

            Log.d(TAG, "verifyDuoxSignature: msg=" + Hex.toHexString(msg));

            // Convert raw R||S to DER
            byte[] sigDER = rawRSToDerSignature(sigRS64);
            if (sigDER == null)
            {
                Log.e(TAG, "verifyDuoxSignature: failed to convert R||S to DER");
                return false;
            }

            // Verify ECDSA-SHA256
            PublicKey pub = uncompressedBytesToECPublicKey(credPubKey65);
            if (pub == null) return false;

            Signature verifier = Signature.getInstance("SHA256withECDSA", new BouncyCastleProvider());
            verifier.initVerify(pub);
            verifier.update(msg);
            boolean result = verifier.verify(sigDER);
            Log.d(TAG, "verifyDuoxSignature: result=" + result);
            return result;
        }
        catch (Exception e)
        {
            Log.e(TAG, "verifyDuoxSignature failed", e);
            return false;
        }
    }
